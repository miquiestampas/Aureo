...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Ruta para exportar resultados de búsqueda a Excel
  app.post("/api/search/excel-data/export", async (req, res) => {
    try {
      console.log("Solicitud de exportación recibida:", req.body);
      ...
      if (!isSimpleSearch) {
        ...
        // Filtro de precio con operador
        if (price !== undefined && price !== null && price !== '') {
          filters.price = parseFloat(price as string);
          filters.priceOperator = priceOperator;
        }
      }
      
      // Ejecutar la búsqueda con los filtros configurados
      console.log(`Realizando búsqueda para exportación ${isSimpleSearch ? 'simple' : 'avanzada'} con:`, { query, filters });
      let results = await storage.searchExcelData(query, filters);
      
      console.log(`Búsqueda para exportación completada. Exportando ${results.length} registros`);
      ...
      if (onlyAlerts && !isSimpleSearch) {
        try {
          ...
          // Filtrar resultados a solo aquellos con alertas
          results = results.filter(record => excelDataIdsWithAlerts.has(record.id));
          console.log(`Filtrado por alertas completado. Exportando ${results.length} registros`);
        } catch (alertError) {
          console.error("Error al filtrar por alertas:", alertError);
          // Continuar con los resultados sin filtrar
        }
      ...
      worksheet.columns = [
        ...
        { header: 'Tienda', key: 'storeCode', width: 15 },
        { header: 'Nº Orden', key: 'orderNumber', width: 15 },
        { header: 'Fecha', key: 'orderDate', width: 15 },
        { header: 'Cliente', key: 'customerName', width: 25 },
        { header: 'Documento', key: 'customerContact', width: 15 },
        { header: 'Dirección', key: 'customerAddress', width: 30 },
        { header: 'Provincia/País', key: 'customerLocation', width: 15 },
        { header: 'Detalles', key: 'itemDetails', width: 40 },
        { header: 'Metales', key: 'metals', width: 15 },
        { header: 'Grabados', key: 'engravings', width: 15 },
        { header: 'Piedras', key: 'stones', width: 15 },
        { header: 'Quilates', key: 'carats', width: 10 },
        { header: 'Peso', key: 'itemWeight', width: 10 },
        { header: 'Precio (€)', key: 'price', width: 12 },
        { header: 'Empeño', key: 'pawnTicket', width: 10 },
      ];
      ...
      results.forEach(record => {
        worksheet.addRow({
          ...
          engravings: record.engravings,
          stones: record.stones,
          carats: record.carats,
          itemWeight: record.itemWeight || '',
          price: record.price,
          pawnTicket: record.pawnTicket ? 'Sí' : 'No'
        });
      });
      ...
      res.setHeader('Content-Disposition', `attachment; filename=Resultados_Busqueda_${new Date().toISOString().split('T')[0]}.xlsx`);
